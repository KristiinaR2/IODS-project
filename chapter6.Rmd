...
The results of the model indicate, that the measurement time (week) was statistically significant explanatory variable (p<.000) for bprs scores. The treatment group was not statistically significant (p=.661).
...
To begin the multilevel analysis of the BPRS data, first the fit the random intercept model for the same two explanatory variables (week & treatment) is fitted. Fitting a random intercept model allows the linear regression fit for each subject to differ in intercept from other subjects.
...
"??? The estimated variance of the subject random effects is quite large (),
indicating the considerable variation in the intercepts of the regression ﬁts of
the individual bprs proﬁles."
...
ggplot(BPRSL, aes(x = week, y = bprs, group = ID)) +
  geom_line(aes(linetype = treatment)) +
  scale_x_continuous(name = "Time (weeks)", breaks = seq(1, 8)) +
