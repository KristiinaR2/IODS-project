...
```{r}
alc2 <- read.table("https://github.com/rsund/IODS-project/raw/master/data/alc.csv", sep = "," , header=TRUE)
str(alc2)
...
# The aggregate data and the data frames
alc_agg <- alc %>%
...
round(prop.table(table(alc$high_use, alc$sex),2)*100)

alc %>% group_by(high_use, sex) %>% summarise(count = n())%>%ungroup()
...
prop.table(table(alc$high_use, alc$famrel))*100

# Plots 
ggplot(data = alc, aes(x = famrel))+ geom_bar()
ggplot(data = alc, aes(x = high_use))+ geom_bar() + facet_wrap(~famrel)

ggplot(alc, aes(x = high_use, y = Medu, col = sex)) + geom_boxplot() + ylab("famrel") + ggtitle("Student alcohol consumption by family relations")
...
prop.table(table(alc$high_use, alc$Medu))*100

# Plots
ggplot(data = alc, aes(x = Medu))+ geom_bar()
ggplot(data = alc, aes(x = high_use))+ geom_bar() + facet_wrap(~Medu)

ggplot(alc, aes(x = high_use, y = Medu, col = sex)) + geom_boxplot() + ylab("Fedu") + ggtitle("Student alcohol consumption by mothers educational level")
...
prop.table(table(alc$high_use, alc$Fedu))*100
...
ggplot(alc, aes(x = high_use, y = Fedu, col = sex)) + geom_boxplot() + ylab("Fedu") + ggtitle("Student alcohol consumption by fathers educational level")
...
a) Present and interpret a summary of the fitted model. 


b) Present and interpret the coefficients of the model as odds ratios and provide confidence intervals for them. 


c) Interpret the results and compare them to your previously stated hypothesis. 
...
m <- glm(high_use ~ sex + famrel + Medu + Fedu, data = alc, family = "binomial")
summary(m)
...
m2 <- glm(high_use ~ sex + famrel, data = alc, family = "binomial")
summary(m2)
...
select(alc, high_use, sex, famrel, prediction) %>% tail(10)
